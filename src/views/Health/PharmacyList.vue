<template>
                  ...
                  <form @submit.prevent="savePharmacy">
                    ...
                    <div class="mb-5">
                      ...
                      <input
                        ...
                        class="form-input"
                      />
                      <div v-if="params.image" class="mt-3">
                        <img :src="params.image" alt="Preview" class="w-20 h-20 object-cover" />
                      </div>
                    </div>
  ...
  function deletePharmacy(pharmacy) {
    Swal.fire({
      title: 'Are you sure?',
      text: `You are about to delete ${pharmacy.name}. This action cannot be undone.`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        if (index !== -1) {
          ...
          Swal.fire('Deleted!', `${pharmacy.name} has been deleted.`, 'success');
        }
      }
    });
  }
  ...
  function savePharmacy() {
    if (params.value.id) {
      ...
      Swal.fire('Updated!', `${params.value.name} has been updated.`, 'success');
    } else {
      params.value.id = pharmacies.value.length + 1;
      pharmacies.value.push({ ...params.value });
      Swal.fire('Added!', `${params.value.name} has been added.`, 'success');
    }
