<template>
    <div class="px-28 py-10 bg-white rounded-lg shadow-md">
        <h1 class="text-2xl font-bold mb-6">Edit Cuisine</h1>
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
            ...
            <div>
                ...
                <select id="restaurant" v-model="formData.restaurantId" required
                    :disabled="loadingRestaurants || loading"
            ...
            <div class="pt-4">
                <button type="submit" :disabled="loading"
                    ...
                    <span v-else>Update Cuisine</span>
                </button>
            </div>
        </form>
    </div>
...
onMounted(async () => {
    const cuisineId = route.params.id as string || route.query.cuisine_id as string;

    if (!cuisineId) {
        toast.error('Cuisine ID is required');
    ...
    try {
        loading.value = true;
        const response = await restaurantService.getCuisineById(cuisineId);
        const cuisine = response.data || response;

        formData.value = {
            restaurantId: cuisine.restaurant_id || cuisine.restaurantId || '',
            name: cuisine.name || ''
        };
    } catch (error: any) {
        console.error('Error loading cuisine:', error);
        const errorMessage = error.response?.data?.message || 'Failed to load cuisine data';
...
const submitForm = async () => {
    ...
    try {
        ...
        await restaurantService.updateCuisine(cuisineId, cuisineData);
        ...
        const errorMessage = error.response?.data?.message || 'Failed to update cuisine';
        toast.error(errorMessage);
    } finally {
        loading.value = false;
    }
