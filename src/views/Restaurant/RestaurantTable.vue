        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="table in paginatedTables" :key="table.id" class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ table.name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500">{{ table.capacity }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500 capitalize">{{ table.restaurant }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-gray-500">
                    ...
                    <span class="font-medium">{{ filteredTables.length }}</span> entries
                </div>
...
<script setup>
    ...
    const Tables = ref([
        ...
        { id: 3, name: 'Table 3', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 4, name: 'Table 4', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 5, name: 'Table 5', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 6, name: 'Table 6', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 7, name: 'Table 7', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 8, name: 'Table 8', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 9, name: 'Table 9', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 10, name: 'Table 10', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 11, name: 'Table 11', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 12, name: 'Table 12', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 13, name: 'Table 13', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 14, name: 'Table 14', capacity: 10, restaurant: 'Italian', status: 'Active' },
        { id: 15, name: 'Table 15', capacity: 10, restaurant: 'Italian', status: 'Active' },
    ]);
    ...
    const filteredTables = computed(() => {
        ...
        return Tables.value.filter(
            (table) =>
                ...
                table.restaurant.toLowerCase().includes(query) ||
                table.status.toLowerCase().includes(query)
        );
    });

    // Pagination calculations
    const totalPages = computed(() => Math.ceil(filteredTables.value.length / perPage.value));

    const pagination = computed(() => {
        const startIndex = (currentPage.value - 1) * perPage.value;
        const endIndex = Math.min(startIndex + perPage.value, filteredTables.value.length);
        return { startIndex, endIndex };
    });

    const paginatedTables = computed(() => {
        return filteredTables.value.slice(pagination.value.startIndex, pagination.value.endIndex);
    });
